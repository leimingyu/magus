...
# GPU Job Table 
#-----------------------------------------------------------------------------#
def FindNextJob(active_job_list, waiting_list, appDur_sorted_dd, gpuOverlapping_dd):
    app1= active_job_list[0]


    gpuovlp_dd =  gpuOverlapping_dd[app1]
    gpuovlp_sorted = sorted(gpuovlp_dd.items(), key=operator.itemgetter(1))
...
def main():
    ...
    #=====================================#
    # compute gpu ovelapping time 
    #=====================================#
    logger.debug("Compute gpu overlapping between apps.")

    # [appDur, cpuTime, gpuTime, threads_max, threads_avg, reg_max, reg_avg, sm_max, sm_avg, trans_max, trans_avg]
    gpuOverlapping_dd = {}
    for app1, metric1 in app2newfeat_dd.iteritems():
        app1_gpuStart = metric1[1]
        app1_gpuEnd   = metric1[2] + app1_gpuStart 
        curApp_gpuovlp= {}

        for app2, metric2 in app2newfeat_dd.iteritems():
            if app1 <> app2:
                app2_gpuStart = metric2[1]
                app2_gpuEnd   = metric2[2] + app2_gpuStart 

                ovlap_time = 0.
                #
                # check overlap timing
                #
                if (app2_gpuStart < app1_gpuStart < app2_gpuEnd) and app2_gpuEnd < app1_gpuEnd:
                    ovlap_time = app2_gpuEnd - app1_gpuStart

                if ( app1_gpuStart < app2_gpuStart < app1_gpuEnd) and  app2_gpuEnd > app1_gpuEnd:
                    ovlap_time = app1_gpuEnd - app2_gpuStart

                if app1_gpuStart > app2_gpuStart  and app1_gpuEnd < app2_gpuEnd:
                    ovlap_time = metric1[2]

                if app2_gpuStart > app1_gpuStart  and app2_gpuEnd < app1_gpuEnd:
                    ovlap_time = metric2[2]

                curApp_gpuovlp[app2] = ovlap_time

        gpuOverlapping_dd[app1] = curApp_gpuovlp 

    logger.debug("Finish computing gpu overlapping time.")
    ...
    for i in xrange(2, apps_num):
        ...
        if Dispatch:
            ...
            if len(active_job_list) == 1:
                ...
                anotherApp = FindNextJob(active_job_list, waiting_list, appDur_sorted, gpuOverlapping_dd)
