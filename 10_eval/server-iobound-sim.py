...
def FindNextJob(waiting_list, appDur_sorted_dd, cpuTime_sorted, active_job_list, app2newfeat_dd, app2app_dist, app_iobound_dd):
    activejob = active_job_list[0]
    activejob_gpuTime = app2newfeat_dd[activejob][2]
    active_iobound = app_iobound_dd[activejob]
    ...
    #
    # continue to select app that least similar 
    #
    dist_dd = app2app_dist[activejob] # get the distance dict
    dist_sorted = sorted(dist_dd.items(), key=operator.itemgetter(1))

    # the sorted in non-decreasing order, use reversed()
    for (sel_appname, _) in reversed(dist_sorted):
        sel_iobound = app_iobound_dd[sel_appname]
        if sel_appname in waiting_list and sel_iobound <> active_iobound: # find 1st app in the list, which is different from the activejob's characteristics
    ...
    for (sel_appname, _) in reversed(dist_sorted):
        if sel_appname in waiting_list:
            app2_name = sel_appname
            break
        ...
        if Dispatch:
            ...
            if len(active_job_list) == 1:
                ...
                anotherApp = FindNextJob(waiting_list, appDur_sorted, cpuTime_sorted,
                        active_job_list, app2newfeat_dd, app2app_dist, app_iobound_dd)
