...
for(int sid=0; sid < num_streams; sid++)
{
      ...
      lud_diagonal<<<1, BLOCK_SIZE, 0, streams[sid]>>>(m, matrix_dim, i);
      lud_perimeter<<<(matrix_dim-i)/BLOCK_SIZE-1, BLOCK_SIZE*2, 0, streams[sid]>>>(m, matrix_dim, i);
      dim3 dimGrid((matrix_dim-i)/BLOCK_SIZE-1, (matrix_dim-i)/BLOCK_SIZE-1);
      lud_internal<<<dimGrid, dimBlock, 0, streams[sid] >>>(m, matrix_dim, i); 
