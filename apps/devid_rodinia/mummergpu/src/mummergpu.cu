...
void loadResultBuffer(MatchContext* ctx)
{
	...
	fprintf(stderr, "Allocating result array for %u queries (%lu bytes) ...", 
					   ...
					   unsigned int* nextqrychar)
{
	...
	*nextqry = qry;
	*nextqrychar = qrychar;
	fprintf(stderr, "Allocing %lu bytes of host memory for %u alignments\n",  alignmentOffset * sizeof(Alignment), numAlignments);
                    ...
                    unsigned int numAlignments)
{
	...
	fprintf(stderr, "Copying %ld bytes to host memory for %u alignments\n",  numAlignments * sizeof(Alignment), numAlignments);
