...
for(int sid=0; sid < num_streams; sid++)
{
        ...
        likelihood_kernel <<< num_blocks, threads_per_block, 0 , streams[sid] >>> (arrayX_GPU, arrayY_GPU, xj_GPU, yj_GPU, CDF_GPU, ind_GPU, objxy_GPU, likelihood_GPU, I_GPU, u_GPU, weights_GPU, Nparticles, countOnes, max_size, k, IszY, Nfr, seed_GPU, partial_sums);

        sum_kernel <<< num_blocks, threads_per_block, 0, streams[sid] >>> (partial_sums, Nparticles);

        normalize_weights_kernel <<< num_blocks, threads_per_block, 0, streams[sid] >>> (weights_GPU, Nparticles, partial_sums, CDF_GPU, u_GPU, seed_GPU);
        
        find_index_kernel <<< num_blocks, threads_per_block, 0, streams[sid] >>> (arrayX_GPU, arrayY_GPU, CDF_GPU, u_GPU, xj_GPU, yj_GPU, weights_GPU, Nparticles);
