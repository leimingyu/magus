...
class dinn(object):
    def __init__(self, sess, n_inputs=90, n_outputs=2, n_hidden =500, 
            n_epochs = 100, batch_size=96, batch_norm_momentum = 0.9, lr = 0.01):
        self.sess = sess
        ...
        with tf.name_scope("dnn"):
            ...
            bn1_act = tf.nn.elu(my_batch_norm_layer(hidden1))
            
            hidden2 = my_dense_layer(bn1_act, n_hidden, name="hidden2")
            bn2_act = tf.nn.elu(my_batch_norm_layer(hidden2))
            
            hidden3 = my_dense_layer(bn2_act, n_hidden, name="hidden3")
            bn3_act = tf.nn.elu(my_batch_norm_layer(hidden3))
    ...
    def test(self, testX, ckpt_model="./dinn/models/dinn_final.ckpt"):
